...
class AccountInvoiceDebit(models.TransientModel):
    ...
    def _get_journal(self):
        ...
        if journal_type == 'sale':
            journal = self.env.ref('debit_credit_note.account_journal_sale_debit_note')
        elif journal_type == 'purchase':
            journal = self.env.ref('debit_credit_note.account_journal_purchase_debit_note')
    ...
    filter_debit = fields.Selection([('debit', 'Create a draft debit note')],
                                    ...
                                    help='Debit Note base on this type. You can not Modify and Cancel if the invoice is already reconciled')
    ...
    def compute_debit(self, mode='debit'):
        ...
        if xml_id:
            result = self.env.ref('debit_credit_note.%s' % (xml_id)).read()[0]
